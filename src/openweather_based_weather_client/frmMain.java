           ...
           try {
                ...
                */
                ...
                if (conn != null) { // Εάν πραγματοποιηθεί σύνδεση με τη βάση δεδομένων
                    ...
                    /*
                     * Έλεγχος για το αν υπάρχουν οι απαιτούμενες πίνακες στη βάση δεδομένων.
                    ...
                    */
                    ...
                    ResultSet rsTables = md2.getColumns(null, null, "tbl_weatherdata", null);
                    
                    // Στην περίπτωση που δεν υπάρχει, τότε θα δημιουργηθεί και θα περιέχει τις παρακάτω στήλες
                    if (rsTables.next() != true) {
                        insertData = conn.prepareStatement("CREATE TABLE tbl_weatherdata ("
                                + "aa BIGINT GENERATED BY DEFAULT AS IDENTITY, "
                        ...
                        insertData.execute(); // Εκτέλεση της SQL εντολής
                        System.out.println("o tbl_weatherdata dimiourgi8ike");
                    }
                    /* Ομοίως και για τον πίνακα tbl_cities ο οποίος θα περιέχει τις πόλεις 
                     * και τους κωδικούς τους, όπως ορίζονται από το openweather
                    */
                    // Έλεγχος για την ύπαρξη του πίνακα tbl_weatherdata
                    rsTables = md2.getColumns(null, null, "tbl_cities", null);

                    // Στην περίπτωση που δεν υπάρχει, τότε θα δημιουργηθεί και θα περιέχει τις παρακάτω στήλες
                    if (rsTables.next() != true) {
                        insertData = conn.prepareStatement("CREATE TABLE tbl_cities ("
                                + "city_id INTEGER NOT NULL, "
                                + "city VARCHAR(40) NOT NULL, "
                                + "PRIMARY KEY (city_ID));");
